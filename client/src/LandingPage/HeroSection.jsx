import React from "react";
import { useTheme } from "../Components/ThemeContext"; // Import useTheme to access theme state

export default function HeroSection() {
  const { theme } = useTheme(); // Access the theme state

  return (
    <div
      className={`relative isolate px-6 lg:px-8 ${
        theme === "dark" ? "bg-gray-900" : "bg-white"
      }`} // Add dark and light background colors
    >
      <div
        aria-hidden="true"
        className="absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-80"
      >
        <div
          style={{
            clipPath:
              "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)",
          }}
          className="relative left-[calc(50%-11rem)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30 sm:left-[calc(50%-30rem)] sm:w-[72.1875rem]"
        />
      </div>
      <div className="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
        <div className="text-center">
          <h1
            className={`text-5xl font-bold tracking-tight ${
              theme === "dark" ? "text-white" : "text-gray-900"
            } sm:text-7xl`}
          >
            Welcome to QuantumLeap
          </h1>
          <p
            className={`mt-8 text-lg font-medium ${
              theme === "dark" ? "text-white" : "text-gray-500"
            } sm:text-xl`}
          >
            Empowering conversations through image recognition. Unlock insights,
            engage users, and transform how you interact with visual data.
          </p>
          <div className="mt-10 flex items-center justify-center gap-x-6">
            <a
              href="#"
              className="rounded-md bg-indigo-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 transition ease-out duration-300"
            >
              Get started
            </a>
            <a
              href="#"
              className={`text-sm font-semibold ${
                theme === "dark"
                  ? "text-white hover:text-gray-400"
                  : "text-gray-900"
              }`}
            >
              Documentation <span aria-hidden="true">→</span>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
}
