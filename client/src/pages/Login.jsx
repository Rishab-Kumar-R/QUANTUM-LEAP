...
function Login() {
  ...
  // Check if the user is already logged in by checking for an auth token
  useEffect(() => {
    const authToken = localStorage.getItem("authToken");
    if (authToken) {
      // If token is found, navigate directly to the Chat page
      navigate("/chat");
    }
  }, [navigate]);
  ...
  const onSubmit = (data) => {
    ...
    if (data.email === hardcodedEmail && data.password === hardcodedPassword) {
      // If credentials match, save the auth token and navigate to the ImageChatRoom
      localStorage.setItem("authToken", "your-auth-token"); // Store the token in localStorage
      navigate("/chat");
    } else {
      // Show toast message if credentials are incorrect
