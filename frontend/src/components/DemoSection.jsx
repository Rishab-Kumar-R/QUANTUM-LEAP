...
const DEMO_MESSAGES = [
	...
	{
		user: true,
		message: "I can see a geometric pattern with a gradient effect. The shape appears to be floating and emitting a subtle glow, suggesting it might be a representation of quantum phenomena or advanced AI visualization."
	},
	{ user: false, message: "Could you explain the pattern in more detail?" },
	{
		user: true,
		message: "The pattern consists of a series of intersecting lines and circles that create a fluid-like motion, resembling the visual output of an advanced neural network at work."
	}
];
...
const DemoSection = () => {
	...
	const runDemoSequence = useCallback(async (mounted) => {
		if (!mounted) return;
	...
	const MessageBubble = ({ message, user }) => (
		<div className="flex items-start space-x-3">
			<div className={`flex-shrink-0 rounded-full p-2 ${user ? "bg-indigo-600" : "bg-indigo-50"}`}>
				{user ? (
			...
			<div className="flex-1 rounded-2xl bg-indigo-50 p-4">
				<p className={`text-sm ${user ? "text-indigo-900" : "text-indigo-600"}`}>
					{message}
					{message.length === 0 && (
	...
	const UploadUI = () => (
		<div className="group cursor-pointer relative w-full h-full rounded-lg border-2 border-dashed 
      ...
      bg-gradient-to-b from-white to-indigo-50">
			<div className="mb-4 animate-bounce">
				<Upload className="h-12 w-12 text-indigo-400 group-hover:text-indigo-500 
          transition-colors duration-200" />
			</div>
			<h3 className="text-lg font-semibold text-indigo-600 mb-2">Upload an Image</h3>
			<p className="text-sm text-gray-500 text-center max-w-xs">
				Drop your image here or click to browse
			</p>
			<div className="mt-4 text-xs text-gray-400">Supports JPG, PNG, JPEG</div>
			<div className="absolute inset-0 bg-indigo-100/0 group-hover:bg-indigo-100/30 
	...
	const ProcessingImage = () => (
		<div className="flex flex-col items-center w-full">
			...
			<div className="mt-4 text-center">
				<p className="text-sm font-medium text-indigo-600">Processing: quantum_pattern.jpg</p>
				<p className="text-xs text-gray-500 mt-1">Analyzing image contents...</p>
			</div>
		</div>
	);
		...
		<section className="py-24 sm:py-32" id="how-we-work">
			<div className="mx-auto max-w-7xl px-6 lg:px-8">
				<header className="mx-auto max-w-2xl text-center mb-16">
					...
					<p className="mt-4 text-lg text-gray-600">Watch how QUANTUM-LEAP processes and understands images in real-time.</p>
				</header>

				<div className="grid md:grid-cols-2 gap-8">
					{/* Image Processing Panel */}
					<div className="relative rounded-xl border border-indigo-900/10 bg-white/50 p-8 shadow-2xl shadow-indigo-600/10">
						...
						<div className="aspect-square flex items-center justify-center">
							...
							{stage >= 1 && (
								<div className="w-full flex items-center justify-center">
									{stage >= 2 ? <ProcessingImage /> : <div className="w-48 h-48 rounded-lg bg-indigo-100 animate-pulse" />}
								</div>
							)}
						</div>
					</div>
